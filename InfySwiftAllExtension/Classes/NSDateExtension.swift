...
extension Date {
    ...
    public func setDateAndTime() -> String {
        let dateFormate = DateFormatter()
        dateFormate.dateFormat = "yyyy-MM-dd HH:mm:ss"
        return dateFormate.string(from: self)
    }
    
    func differenceinSecond() -> Int {
    ...
    func getYesterday() -> Date {
        return  Calendar.current.date(byAdding: .day, value: -1, to: self as Date)!
    }
    
    func getBeforeSevenDayeDate() -> Date {
        return  Calendar.current.date(byAdding: .day, value: -7, to: self as Date)!
    }
    func getAfterSevenDayeDate() -> Date {
        return  Calendar.current.date(byAdding: .day, value: 7, to: self as Date)!
    }
    func getDayByValue(value : Int) -> Date{
        return  Calendar.current.date(byAdding: .day, value: value , to: self as Date)!
    }
    func getBeforeThirtyDayeDate() -> Date {
        return  Calendar.current.date(byAdding: .day, value: -30, to: self as Date)!
    }
    func getAfterThirtyDayeDate() -> Date {
        return  Calendar.current.date(byAdding: .day, value: 30, to: self as Date)!
    }
    func getBeforeThreeMonthDate() -> Date{
        return  Calendar.current.date(byAdding: .month, value: -3, to: self as Date)!
    }
    func getPreviousWeek() -> Date {
        return  Calendar.current.date(byAdding: .day, value: -6, to: self as Date)!
    }
    ...
    func getDifferenceDate(setDate : Date) -> String{
        
        let formatter = DateComponentsFormatter()
        formatter.unitsStyle = .positional
        formatter.allowedUnits = [.day]
        formatter.zeroFormattingBehavior = .pad
        let dateString = formatter.string(from: setDate, to: Date())!
        
        let string = dateString.substring(to: dateString.index(dateString.endIndex, offsetBy: -1))
        if Int(string)! == 0{
            formatter.allowedUnits = [.hour]
            let time = formatter.string(from: setDate, to: Date())!
            if Int(time)! > 0{
                return "\(formatter.string(from: setDate, to: Date())!)h"
            }else{
                formatter.allowedUnits = [.minute]
                let minute = formatter.string(from: setDate, to: Date())!
                if Int(minute)! > 0{
                    return "\(formatter.string(from: setDate, to: Date())!)mi"
                }else{
                    formatter.allowedUnits = [.second]
                    return "\(formatter.string(from: setDate, to: Date())!)s"
                }
            }
        }else if Int(string)! > 30{
            formatter.allowedUnits = [.month]
            return formatter.string(from: setDate, to: Date())!
        }else{
            return formatter.string(from: setDate, to: Date())!
        }
    }
    func getCommperDateString() -> String {
        let dateFormater = DateFormatter()
        dateFormater.dateFormat = "yyyyMMdd"
        return dateFormater.string(from: self as Date)
    }
    func getDateToDate() -> Date {
        let dateFormate = DateFormatter()
        dateFormate.dateFormat = "yyyy-MM-dd"
        let date = dateFormate.string(from: self as Date)
        dateFormate.dateFormat = "yyyy-MM-dd"
